...
export function useSmartPagination<T = any>(
  ...
  const [dataCache, setDataCache] = useState<Map<number, T[]>>(new Map())

  // Update pagination state from API response
  const updatePaginationState = useCallback((response: PaginatedApiResponse<T>) => {
  ...
  const loadPage = useCallback(async (
    page: number,
    fetchFunction: (page: number, limit: number) => Promise<PaginatedApiResponse<T>>
  ) => {
    console.log('📄 loadPage called:', { page, hasPage: paginationState.loadedPages.has(page) })
    ...
    try {
      // Fetch the page data
      const response = await fetchFunction(page, paginationState.pageSize)
  ...
  }, [paginationState.loadedPages, paginationState.pageSize, updatePaginationState])
  ...
  }, [paginationState.hasNext, paginationState.isLoading, paginationState.currentPage, loadPage])
  ...
  }, [paginationState.hasPrev, paginationState.isLoading, paginationState.currentPage, loadPage])
  ...
  const getPageData = useCallback((page: number) => {
  ...
  return {
    ...
    isCurrentPageLoaded: paginationState.loadedPages.has(paginationState.currentPage),
  }
}
