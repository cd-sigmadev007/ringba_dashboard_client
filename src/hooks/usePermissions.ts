...
export function usePermissions(): UserPermissions {
            ...
            if (isAuthenticated) {
                try {
                    ...
                    console.log('🔍 Looking for role in:', {
                        ...
                        namespacedRole:
                            tokenClaims?.[
                                `${import.meta.env.VITE_AUTH0_AUDIENCE || ''}/role`
                            ],
                    })
    ...
    const namespacedCampaignIds = audience
        ? claims[`${audience}/campaign_ids`]
        : null
    ...
    const campaign_ids = (namespacedCampaignIds ||
        claims.campaign_ids ||
        []) as Array<string>
    ...
    if (role) {
        ...
        console.warn(
            '⚠️ No role found in claims. Available keys:',
            Object.keys(claims || {})
        )
    }
