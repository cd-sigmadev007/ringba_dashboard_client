...
export const TableRenderer: React.FC<Props> = ({ result }) => {
    ...
    const thCls = clsx(
        ...
        isDark ? 'text-[#4A6080] hover:text-[#B0C4DE]' : 'text-gray-500 hover:text-gray-700',
    )
    const tdCls = clsx(
        'px-4 py-2.5 text-sm font-mono whitespace-nowrap border-b',
        isDark ? 'border-[#1E3A5F] text-[#B0C4DE]' : 'border-gray-100 text-gray-700',
    )

    return (
        <div className="overflow-auto rounded-xl border" style={{ maxHeight: 460 }}>
            <table className={clsx('w-full text-sm border-collapse', isDark ? 'bg-[#0A1929]' : 'bg-white')}>
                <thead className={clsx('sticky top-0 z-10', isDark ? 'bg-[#0D2137]' : 'bg-gray-50')}>
                    <tr>
                ...
                <tbody>
                    {sorted.map((row, ri) => (
                        <tr key={ri} className={clsx(
                            'transition-colors',
                            isDark ? 'hover:bg-[#1E3A5F]/40' : 'hover:bg-gray-50',
                        )}>
                            {result.columns.map((col) => {
                                const v = row[col]
                                ...
                                return (
                                    <td key={col} className={tdCls} title={display}>
                                        <span className="block max-w-xs truncate">{display}</span>
                                    </td>
                                )
                            })}
                        </tr>
