...
export default function AllowedEmailsList({
    ...
    const handleRemove = async (emailId: string, email: string) => {
        if (
            window.confirm(
                `Are you sure you want to remove ${email} from the allowlist?`
            )
        ) {
            try {
    ...
    const columns = useMemo<Array<ColumnDef<AllowedEmail>>>(
        () => [
            {
                accessorKey: 'email',
                header: 'Email',
                    ...
                    return (
                        <div className="flex gap-2">
                            <Button
                                variant="ghost"
                                onClick={() =>
                                    handleRemove(email.id, email.email)
                                }
