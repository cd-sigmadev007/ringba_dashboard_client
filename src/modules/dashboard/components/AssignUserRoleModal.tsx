...
export default function AssignUserRoleModal({
  ...
  const [selectedRole, setSelectedRole] = useState<'super_admin' | 'org_admin' | 'user'>('user')
  ...
  // Reset org selection when role changes to super_admin
  useEffect(() => {
    if (selectedRole === 'super_admin') {
      setSelectedOrgId(null)
    } else if (!selectedOrgId && user?.org_id) {
      // Preserve existing org if available when switching to org_admin/user
      setSelectedOrgId(user.org_id)
    }
  }, [selectedRole, user])
  ...
  return (
    ...
    >
      {!user ? (
        <p className="text-gray-600 dark:text-gray-400">No user selected</p>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setSelectedRole(
                  e.target.value as 'super_admin' | 'org_admin' | 'user'
                )
              }
            ...
            {selectedRole === 'super_admin' && (
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                Super admins are not assigned to any organization
              </p>
            )}
          </div>
            ...
            <div>
              ...
              {orgsLoading ? (
                ...
                <select
                  ...
                  disabled={assignMutation.isPending || organizations.length === 0}
                  className="w-full px-[15px] py-[10px] rounded-[7px] border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-0 focus:border-[#007FFF] disabled:opacity-50 disabled:cursor-not-allowed"
                  required
                >
                  <option value="">
          ...
          <div className="flex justify-end gap-3 pt-4">
            ...
            >
              {assignMutation.isPending ? 'Updating...' : 'Update Role'}
            </button>
          </div>
        </form>
      )}
