...
export default function AssignUserToOrgModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to assign user to organization')
    }
  }
      ...
      {!user ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            {user.role === 'super_admin' && (
              <p className="text-sm text-yellow-600 dark:text-yellow-400 mb-4">
                Note: Super admins cannot be assigned to organizations
              </p>
            )}
          </div>
          ...
          <div className="flex justify-end gap-3 pt-4">
            ...
            >
              {assignMutation.isPending ? 'Updating...' : 'Update Organization'}
            </button>
          </div>
        </form>
      )}
