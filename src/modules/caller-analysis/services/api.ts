...
class CallerApiService {
  ...
  async getAllCallers(params: CallerQueryParams = {}): Promise<PaginatedResponse<FrontendCallerData>> {
    const queryParams = new URLSearchParams();
    
    if (params.page) queryParams.append('page', params.page.toString());
    if (params.limit) queryParams.append('limit', params.limit.toString());
    if (params.search) queryParams.append('search', params.search);
    if (params.campaign) params.campaign.forEach(c => queryParams.append('campaign', c));
    if (params.status) params.status.forEach(s => queryParams.append('status', s));
    if (params.dateFrom) queryParams.append('dateFrom', params.dateFrom);
    if (params.dateTo) queryParams.append('dateTo', params.dateTo);
    if (params.durationMin) queryParams.append('durationMin', params.durationMin.toString());
    if (params.durationMax) queryParams.append('durationMax', params.durationMax.toString());
  ...
  async getTableSchema(): Promise<any> {
    const response = await this.makeRequest<ApiResponse<any>>('/callers/schema');
    return response.data!;
  }

  // Health check
  async healthCheck(): Promise<{ status: string; timestamp: string }> {
    const response = await this.makeRequest<ApiResponse<{ status: string; timestamp: string }>>('/callers/health');
    return response.data!;
  }
