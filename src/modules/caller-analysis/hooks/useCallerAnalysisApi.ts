...
export const useCallerAnalysisApi = () => {
  ...
  // Query key factory
  const getCallersQueryKey = (params: CallerQueryParams = {}) => ['callers', params];

  // Get all callers with filtering and pagination
  const useGetAllCallers = (filters: FilterState, page: number = 1, limit: number = 10) => {
  ...
  const convertApiDataToCallData = (apiData: FrontendCallerData[]): CallData[] => {
